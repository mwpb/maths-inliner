...
var brd = JXG.JSXGraph.initBoard('box', {boundingbox: [-10,8,10,-8], axis:false,showCopyright:false,showNavigation:false});
...
var textPerimeter = brd.create('text',[-3,-6,function(){return "Perimeter = "+perimeter().toFixed(2)}],{fontSize:18})
...
var allHiddenTexts = ge(["main0","main1","main2","main3"]);
function hideTexts(){
	for (var i=0;i<allHiddenTexts.length;i++){
		console.log("hiding: "+allHiddenTexts[i].toString());
...
function increment(inc) {
	...
	for (var i=0;i<visTexts.length;i++){
		visTexts[i].style.display = 'block';
	}
}
increment(-1);